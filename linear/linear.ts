...
async function main(): Promise<void> {
  ...
  try {
    switch (command) {
      ...
      case 'projects': {
        const all = Boolean(params.all)
        const json = JSON.stringify(await listProjects(params.teamId as string | undefined, all), null, 2)
        console.log(json)
        return
      }
      case 'issues': {
        const json = JSON.stringify(await listIssues(params.teamId as string | undefined, params.projectId as string | undefined), null, 2)
        console.log(json)
        return
      }
      case 'issue': {
      ...
      default:
        console.error(
          JSON.stringify({
            ...
            supported: ['teams', 'projects', 'issues', 'issue', 'me', 'set-userId'],
          }),
        )
        process.exit(1)
        return
...
const DEBUG_ENABLED: boolean = Boolean(parsedGlobalArgs.debug) || process.env.DEBUG_LINEAR === '1'
...
async function collectAllNodes<T>(
  ...
  while (currentConn.pageInfo?.hasNextPage && typeof currentConn.next === 'function') {
...
async function listTeams(showAll = false): Promise<unknown> {
  ...
  const teamMemberships = await collectAllNodes(Promise.resolve(teamMembershipsConnection))
...
async function listProjects(teamIdArg: string | undefined, showAll = false): Promise<unknown> {
  ...
  if (teamIdArg) {
    ...
    debug(`Projects returned: ${filtered.length} (filtered by teamId: ${teamIdArg})`)
  ...
  const filtered = projectsWithTeams.filter((p) => p.teamId && myTeamIds.has(p.teamId))
  debug(`Projects returned: ${filtered.length} (filtered by current team membership)`)
  return filtered
}

async function listIssues(teamId: string | undefined, projectId: string | undefined): Promise<unknown> {
  if (!teamId && !projectId) {
...
async function getIssue(issueId: string): Promise<unknown> {
  if (!issueId) throw new Error("Issue ID is required for issue (e.g., 'VIT-364' or '--id VIT-364')")
...
async function setUserId(): Promise<unknown> {
  ...
  try {
    ...
    const userIdLineIndex = lines.findIndex((line) => line.trim().startsWith('export LINEAR_USER_ID='))
    ...
    if (userIdLineIndex >= 0) {
      ...
      const apiKeyIndex = lines.findIndex((line) => line.trim().startsWith('export LINEAR_API_KEY='))
